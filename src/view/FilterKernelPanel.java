...
/**
 * <p>
 * Title: FilterKernelPanel
 * </p>
 * <p>
 * Description:
 * </p>
 * <p>
 * Copyright: Copyright (c) 2003 Sébastien Bois, Eric Paquette
 * </p>
 * <p>
...
public class FilterKernelPanel extends JPanel implements ObserverIF {
	...
	public FilterKernelPanel(TransformersIndex ti) {
		...
		_handlingBorderComboBox = new JComboBox(
				KernelModel.HANDLING_BORDER_ARRAY);
		...
		// Initialize the Handling Border combo box to the default value of the
		// handling border combo box
		...
		_handlingBorderComboBox.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent ae) {
				FilterKernelPanel.this.ti.getTheFilter().setBorder(
						(String) _handlingBorderComboBox.getSelectedItem());
			}
		});

		_clampComboBox.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent ae) {
				FilterKernelPanel.this.ti.getTheFilter().setClamp(
						(String) _clampComboBox.getSelectedItem());
			}
		});

		_filterTypeComboBox.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent ae) {
				setFilter((String) _filterTypeComboBox.getSelectedItem());
			}
		});

		_handlingBorderLabel.setBorder(BorderFactory
				.createEtchedBorder(EtchedBorder.LOWERED));
		_rangeClampLabel.setBorder(BorderFactory
				.createEtchedBorder(EtchedBorder.LOWERED));
		_filterTypeLabel.setBorder(BorderFactory
				.createEtchedBorder(EtchedBorder.LOWERED));
	...
	private void setFilter(String string) {
		// valeurs tirées de
		// https://cours.etsmtl.ca/gti410/private/Acetates/GTI410_05_filtrageLineaire.pdf
		int index = 0;
		...
		case 1: // Mean filter
		{
			float meanKernel[][] = { { 1, 1, 1 }, { 1, 1, 1 }, { 1, 1, 1 } };
			_kernelPanel.setKernelValues(meanKernel);
		}
			break;
		case 2: // Gaussian filter
		{
			float meanKernel[][] = { { 1, 2, 1 }, { 2, 4, 2 }, { 1, 2, 1 } };
			_kernelPanel.setKernelValues(meanKernel);
		}
			break;
		case 3: // 4-Neighbour Laplacian
		{
			float meanKernel[][] = { { 0, 1, 0 }, { 1, -4, 1 }, { 0, 1, 0 } };
			_kernelPanel.setKernelValues(meanKernel);
		}
			break;
		case 4: // 8-Neighbour Laplacian
		{
			float meanKernel[][] = { { 1, 1, 1 }, { 1, -8, 1 }, { 1, 1, 1 } };
			_kernelPanel.setKernelValues(meanKernel);
		}
			break;
		case 5: // Prewitt Horiz
		{
			float meanKernel[][] = { { -1, 0, 1 }, { -1, 0, 1 }, { -1, 0, 1 } };
			_kernelPanel.setKernelValues(meanKernel);
		}
			break;
		case 6: // Prewitt Vert
		{
			float meanKernel[][] = { { -1, -1, -1 }, { 0, 0, 0 }, { 1, 1, 1 } };
			_kernelPanel.setKernelValues(meanKernel);
		}
			break;
		case 7: // Sobel Horiz
		{
			float meanKernel[][] = { { -1, 0, 1 }, { -2, 0, 2 }, { -1, 0, 1 } };
			_kernelPanel.setKernelValues(meanKernel);
		}
			break;
		case 8: // Sobel Vert
		{
			float meanKernel[][] = { { -1, -2, -1 }, { 0, 0, 0 }, { 1, 2, 1 } };
			_kernelPanel.setKernelValues(meanKernel);
		}
			break;
		case 9: // Roberts 45 degrees
		{
			float meanKernel[][] = { { 0, 0, 0 }, { 1, -1, 0 }, { 0, 0, 0 } };
			_kernelPanel.setKernelValues(meanKernel);
		}
			break;
		case 10: // Roberts -45 degrees
		{
			float meanKernel[][] = { { 10, 2, 3 }, { 4, 5, 6 }, { 7, 8, 9 } };
			_kernelPanel.setKernelValues(meanKernel);
		}
			break;
		case 0: // Custom
		...
		// The following is needed because as we were updated, we automatically
		// switched to Custom.
